...
function extractPropertyValue(propValue: any): string {
...
function findNestedInstances(node: SceneNode): InstanceNode[] {
...
async function groupInstancesByComponent(instances: InstanceNode[]): Promise<ComponentInfo[]> {
...
async function cleanupExistingInteraction(interactionId: string) {
...
async function createInteraction(interactionData: Interaction) {
  try {
    if (!variableCollection) {
      await setupVariableCollection();
    }
    ...
    figma.ui.postMessage({
      ...
      message: `Interaction created successfully for ${componentData.find(c => c.id === interactionData.component)?.name}`
    });
...
async function storeInteractionData(interactionData: Interaction) {
  try {
...
async function retrieveInteractionData(componentId: string): Promise<Interaction | null> {
  try {
...
async function findVariantProperty(
  primaryProp: string, 
  instances: InstanceNode[]
): Promise<{ actualPropertyName: string | null; isVariantProperty: boolean }> {
...
async function createAndBindVariables(
  interaction: Interaction,
  component: ComponentInfo,
  actualPropertyName: string,
  primaryProp: string
): Promise<{ instanceVars: Variable[]; originalStates: string[] }> {
...
async function applyInteractionToInstances(interaction: Interaction, primaryVar: Variable, conditionalVars: Variable[]) {
  try {
    // Find the component
      ...
      try {
        if (actualPropertyName && isVariantProperty && instanceVars.length > 0) {
          ...
          for (let j = 0; j < component.instances.length; j++) {
            if (i === j) continue; // Skip the clicked instance
